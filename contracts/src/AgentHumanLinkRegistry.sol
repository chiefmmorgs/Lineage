...
contract AgentHumanLinkRegistry is EIP712 {
    ...
    // Duplicate prevention: keccak256(agentWallet, agentTokenId, humanWallet) => active linkId (0 = none)
    mapping(bytes32 => uint256) private _activeLinkByParties;
    ...
    function createVerifiedLink(
        ...
        bytes32 partyKey = _partyKey(agentWallet, agentTokenId, msg.sender);
        uint256 existingLinkId = _activeLinkByParties[partyKey];
        if (existingLinkId != 0) {
            require(!isLinkActive(existingLinkId), "Active link already exists between these parties");
        }
        ...
        address resolvedHuman = hasHuman ? humanSigner : msg.sender;
        _humanLinks[resolvedHuman].push(linkId);
        _profileLinks[ethosProfileId].push(linkId);
        _activeLinkByParties[_partyKey(agentWallet, agentTokenId, resolvedHuman)] = linkId;
    ...
    /**
     * @notice Total number of links ever created.
     */
    ...
    function _partyKey(address agentWallet, uint256 agentTokenId, address humanWallet) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(agentWallet, agentTokenId, humanWallet));
    }
}
